...
// 「次に来る25日 10:00（日本時間）」のUnixタイムスタンプ（秒）を計算する。
// 今日が25日より前ならその月の25日、25日を過ぎていれば翌月の25日を返す。
// これを Stripe サブスクリプションの trial_end に設定することで、
// 契約者ごとにバラバラだった請求日を「全員毎月25日」に統一する（特商法表記と一致させるため）。
function getNextBillingCycleAnchor() {
  const now = new Date()
  const nowJst = new Date(now.getTime() + 9 * 60 * 60 * 1000)
  const year = nowJst.getUTCFullYear()
  const month = nowJst.getUTCMonth()
  // JST 25日10:00 = UTC 25日01:00
  let candidate = new Date(Date.UTC(year, month, 25, 1, 0, 0))
  if (candidate.getTime() <= now.getTime()) {
    candidate = new Date(Date.UTC(year, month + 1, 25, 1, 0, 0))
  }
  return Math.floor(candidate.getTime() / 1000)
}

// サブスクリプションの次回請求日を「次の25日」へ付け替える（追加課金なし）
// ※Stripe仕様上、既存サブスクリプションのbilling_cycle_anchorは直接更新できない
//   （update時に指定できるのは 'now' か 'unchanged' のみで、任意の日付は指定不可）。
//   そのため trial_end を「次の25日」に設定することで、間接的に
//   billing_cycle_anchor をそのタイミングへ付け替える（実際の課金は発生させない）。
async function alignBillingCycleToThe25th(stripe, subscriptionId) {
  try {
    const anchor = getNextBillingCycleAnchor()
    await stripe.subscriptions.update(subscriptionId, {
      trial_end: anchor,
      proration_behavior: 'none',
    })
  } catch (err) {
    console.error('請求日を毎月25日へ調整する処理に失敗しました。手動確認が必要です', err)
  }
}
...
export default async function handler(req, res) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    try {
      ...
      if (!oversold && session.subscription) {
        // 支払い確定＆満枠でもない場合のみ、次回以降の請求日を毎月25日に統一する
        await alignBillingCycleToThe25th(stripe, session.subscription)
      }
