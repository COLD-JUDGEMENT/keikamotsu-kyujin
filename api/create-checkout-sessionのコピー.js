...
// 料金体系（確定仕様）：円建て・月額・税別。Stripeは「銭」単位ではなく「円そのもの」を
// 最小単位として扱う設定にする（JPYはゼロ decimal通貨）。
// 消費税（10%）は自動計算(Stripe Tax)を使わず、下記でline_itemsにもう1行として
// 明示的に追加する方式にしている（国内・単一税率のみのシンプルな運用のため）。
const PLAN_PRICES = {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const taxAmount = Math.round(planInfo.amount * TAX_RATE)
