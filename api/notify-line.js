// api/notify-line.js
// 新規掲載申込・AI審査グレー/NG判定・満枠返金の際に、LINEへ通知を送る。
// フロントエンド（App.jsx）やAI審査（api/screen-listing.js）、Stripe Webhook（api/stripe-webhook.js）
// から呼び出される想定。
//
// 必要なVercel環境変数：
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (isGray || isNg) {
      ...
      bodyLines.push(`エリア：${areaLabel || '(不明)'}`)
      bodyLines.push('決済タイミングが重なり満枠を超えたため、自動的に返金・解約しました。企業様への連絡をお願いします。')
    } else {
      bodyLines.push(`エリア：${areaLabel || '(不明)'}`)
    }
    ...
    if (isNg) {
      ...
      // 返金済みなので承認/却下は不要。管理画面で状況確認するボタンのみ
      actions = [
        { type: 'uri', label: '管理画面を開く', uri: 'https://keikamotsu-kyujin.vercel.app/?admin=1' },
      ]
    } else {
