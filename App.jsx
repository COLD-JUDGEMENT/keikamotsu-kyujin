...
.kk-checkbox-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }
.kk-checkbox-item { display: flex; align-items: center; gap: 6px; font-size: 13px; border: 1px solid #ccc; border-radius: 5px; padding: 8px 10px; cursor: pointer; }
.kk-checkbox-item input { margin: 0; }
...
const VEHICLE_CONDITION_LABELS = { own_required: '持ち込み必須', rental_available: '貸与あり', either: 'どちらでも可' }
...
export default function App() {
  ...
  const toggleWorkDaysOption = (optionId) => {
    setPostForm(prev => {
      const has = prev.workDays.includes(optionId)
      const workDays = has ? prev.workDays.filter(id => id !== optionId) : [...prev.workDays, optionId]
  ...
  const handlePostSubmit = async (e) => {
    ...
    try {
      ...
      const writePromise = addDoc(collection(db, 'listingRequests'), {
        ...
        jobTypeOther: postForm.jobType === 'other' ? postForm.jobTypeOther : '',
        ...
        paymentTermsOther: postForm.paymentTerms === 'other' ? postForm.paymentTermsOther : '',
          ...
          {filteredListings.map(listing => (
            <div key={listing.id} className="kk-admin-card">
              ...
              <div className="kk-admin-card-row">
                業務形態：{listing.jobType === 'other' ? listing.jobTypeOther : (JOB_TYPE_LABELS[listing.jobType] || '未回答')}
                　車両条件：{VEHICLE_CONDITION_LABELS[listing.vehicleCondition] || '未回答'}
                {listing.vehicleConditionDetail && `（${listing.vehicleConditionDetail}）`}
              </div>
              <div className="kk-admin-card-row">
                支払条件：{listing.paymentTerms === 'other' ? listing.paymentTermsOther : (PAYMENT_TERMS_LABELS[listing.paymentTerms] || '未回答')}
          ...
          <section id="postForm" className="kk-postform-section">
            ...
            {postStatus === 'success' ? (
              ...
              <form className="kk-postform" onSubmit={handlePostSubmit}>
                ...
                <div className="kk-form-group">
                  <label>業務形態<span className="kk-required">必須</span></label>
                  <select required value={postForm.jobType} onChange={e=>updatePostForm('jobType', e.target.value)}>
                    <option value="">選択してください</option>
                    <option value="commission">業務委託</option>
                    <option value="spot">スポット</option>
                    <option value="contract">契約社員</option>
                    <option value="employee">社員</option>
                    <option value="other">その他</option>
                  </select>
                  {postForm.jobType === 'other' && (
                ...
                <div className="kk-form-group">
                  ...
                  <select required value={postForm.vehicleCondition} onChange={e=>updatePostForm('vehicleCondition', e.target.value)}>
                    <option value="">選択してください</option>
                    <option value="own_required">持ち込み必須</option>
                ...
                <div className="kk-form-group">
                  <label>報酬の支払条件<span className="kk-required">必須</span></label>
                  <select required value={postForm.paymentTerms} onChange={e=>updatePostForm('paymentTerms', e.target.value)}>
                    <option value="">選択してください</option>
                    <option value="end_of_month_next">月末締め翌月末払い</option>
                    <option value="end_of_month_next_next">月末締め翌々月末払い</option>
                    <option value="weekly">週払い</option>
                    <option value="daily">日払い</option>
                    <option value="other">その他</option>
                  </select>
                  {postForm.paymentTerms === 'other' && (
                ...
                <div className="kk-form-group">
                  <label>勤務日数<span className="kk-required">必須</span></label>
                  <div className="kk-checkbox-grid">
                    {WORK_DAYS_OPTIONS.map(opt => (
                      <label key={opt.id} className="kk-checkbox-item">
                        <input
                          type="checkbox"
                          checked={postForm.workDays.includes(opt.id)}
                          onChange={()=>toggleWorkDaysOption(opt.id)}
                        />
                        <span>{opt.label}</span>
                      </label>
                    ))}
                ...
                <button type="submit" className="kk-btn-post kk-form-submit" disabled={!postForm.agree || postStatus === 'submitting'}>
