...
import {
  ...
  query, orderBy, onSnapshot, doc, updateDoc, where,
} from 'firebase/firestore'
import {
  getAuth, signInWithEmailAndPassword, signOut, onAuthStateChanged,
} from 'firebase/auth'

// reCAPTCHA v3のサイトキー（公開値。シークレットキーはサーバー側のみで使用）
const RECAPTCHA_SITE_KEY = '6LdDvogtAAAAAFgC9kCAAa52nEwkS-UD73KOhKJe'
...
const MAP_VIEWBOX = "-14 -14 354 395"
const OKI_BOX = { x: 5, y: 10, w: 150, h: 99.2 }
const prefecturePaths = [
  ...
  { id: "okinawa", name: "沖縄県", regionId: "kyushu", d: "M24.6,101.7 L26.6,102.1 L25.6,104.2 L22.4,103.7 L23.9,101.2 L24.6,101.7Z M11.1,100.1 L11.0,100.8 L10.0,100.7 L11.1,100.1Z M33.2,98.1 L31.8,102.2 L30.1,102.6 L29.0,100.9 L29.9,100.2 L31.3,100.6 L33.2,98.1Z M50.9,94.4 L52.0,95.1 L48.7,95.3 L48.7,92.5 L50.9,94.4Z M148.8,74.0 L148.7,75.1 L148.1,74.9 L148.8,74.0Z M74.6,65.2 L74.3,66.1 L72.7,64.8 L73.6,64.4 L74.6,65.2Z M100.1,56.9 L99.6,58.7 L97.1,60.2 L96.8,61.4 L95.4,61.5 L93.5,63.4 L92.0,63.5 L93.1,65.9 L91.6,65.8 L90.6,67.7 L91.5,68.2 L91.1,69.1 L88.7,70.1 L88.4,67.7 L90.2,65.7 L89.8,63.7 L93.8,61.2 L92.7,60.4 L92.4,58.8 L94.3,58.8 L94.5,59.9 L96.3,59.2 L98.7,55.2 L100.1,56.9Z M106.2,44.6 L104.7,46.3 L103.3,46.2 L103.5,45.3 L106.2,44.6Z M111.0,38.0 L111.6,39.0 L110.0,40.3 L109.3,39.3 L109.5,36.4 L110.6,36.4 L111.0,38.0Z M116.1,32.2 L117.0,32.8 L114.8,32.6 L114.2,30.9 L115.6,31.2 L116.1,32.2Z M128.3,27.5 L127.8,29.0 L126.4,28.9 L128.3,27.5Z M123.0,25.9 L123.1,26.5 L121.5,27.0 L119.5,29.1 L118.6,29.0 L118.2,29.8 L119.0,30.4 L117.0,31.5 L117.4,32.2 L115.6,30.2 L113.6,29.8 L115.7,29.7 L116.0,29.2 L115.0,28.8 L115.6,28.3 L119.1,26.6 L119.8,27.1 L121.6,25.5 L121.7,26.4 L122.4,25.0 L123.0,25.9Z M91.3,58.0 L90.4,58.4 L91.3,58.0Z M94.4,51.4 L93.2,52.9 L94.4,51.4Z M149.4,72.6 L149.9,73.1 L149.4,72.6Z M102.0,52.2 L101.1,52.3 L102.0,52.2Z M47.8,93.2 L47.8,94.0 L46.7,93.8 L47.1,93.0 L47.8,93.2Z M83.9,67.7 L83.5,68.8 L83.9,67.7Z M20.1,76.2 L19.6,76.0 L20.1,76.2Z" },
]
...
.kk-premium-ad { background: linear-gradient(135deg, #fff8e1, #ffecb3); border: 2px solid #f9a825; border-radius: 10px; text-align: center; padding: 22px 16px; margin: 14px 16px; cursor: pointer; box-shadow: 0 2px 10px rgba(249,168,37,0.25); transition: transform 0.12s; }
.kk-premium-ad:hover { transform: translateY(-2px); }
.kk-premium-label { font-size: 13px; color: #f57f17; font-weight: 700; margin-bottom: 6px; letter-spacing: 0.5px; }
.kk-premium-company { font-size: 22px; font-weight: 800; color: #222; margin-bottom: 6px; }
.kk-premium-jobtitle { font-size: 14px; color: #555; font-weight: 600; margin-bottom: 8px; }
.kk-premium-highlights { display: flex; gap: 8px; justify-content: center; flex-wrap: wrap; margin-bottom: 8px; }
.kk-premium-chip { background: #fff; border: 1px solid #f9a825; border-radius: 20px; padding: 5px 14px; font-size: 13px; font-weight: 700; color: #333; }
.kk-premium-chip-pay { background: #f57f17; color: #fff; border-color: #f57f17; }
.kk-premium-desc { font-size: 13px; color: #666; margin-top: 4px; font-weight: 600; }
...
.kk-btn-post-outline { background: #fff; color: #1a237e; border: 1px solid #1a237e; }
.kk-jobdetail-section { background: #fff; border-radius: 8px; padding: 16px; }
.kk-jobdetail-header { margin-bottom: 12px; }
.kk-jobdetail-title { font-size: 18px; margin: 6px 0 4px; }
.kk-jobdetail-company { font-size: 13px; color: #666; }
.kk-detail-row { display: flex; gap: 10px; padding: 6px 0; border-bottom: 1px solid #f2f2f2; font-size: 13px; }
.kk-detail-label { flex: 0 0 120px; color: #888; font-weight: 600; }
.kk-detail-text { font-size: 13px; line-height: 1.7; white-space: pre-wrap; color: #333; margin: 4px 0 12px; }
.kk-jobdetail-contact { display: flex; gap: 10px; flex-wrap: wrap; margin-top: 10px; }
.kk-about-section { background: #fff; border-radius: 8px; padding: 16px; margin-bottom: 16px; }
.kk-about-box { }
.kk-about-list { font-size: 13px; line-height: 1.9; color: #333; padding-left: 18px; margin: 10px 0 14px; }
.kk-price-section { background: #fff; border-radius: 8px; padding: 16px; margin-bottom: 16px; }
.kk-price-grid { display: flex; gap: 12px; flex-wrap: wrap; margin: 12px 0; }
.kk-price-card { flex: 1 1 200px; border: 1px solid #e0e0e0; border-radius: 8px; padding: 14px; }
.kk-price-card-premium { border-color: #f57f17; background: #fffbf0; }
.kk-price-name { font-size: 13px; font-weight: 700; color: #1a237e; margin-bottom: 4px; }
.kk-price-amount { font-size: 22px; font-weight: 700; color: #222; margin-bottom: 10px; }
.kk-price-unit { font-size: 11px; font-weight: 400; color: #888; }
.kk-price-features { font-size: 12px; line-height: 1.8; color: #555; padding-left: 16px; }
...
.kk-ai-screening-badge { font-size: 11px; padding: 6px 10px; border-radius: 5px; margin-bottom: 8px; font-weight: 600; }
.kk-ai-screening-gray { background: #fff8e1; color: #f57f17; }
.kk-ai-screening-ng { background: #ffebee; color: #c62828; }
...
.kk-admin-card-actions button { border: none; border-radius: 5px; padding: 12px 16px; font-size: 13px; font-weight: 600; cursor: pointer; min-height: 44px; flex: 1; }
...
const PAYMENT_STATUS_LABELS = { not_required: '対象外（無料）', unpaid: '未払い', paid: '支払い済み', payment_failed: '支払い失敗', cancelled: '解約済み', oversold_refund_pending: '満枠のため自動返金済み（要対応）' }
...
export default function App() {
  ...
  const [publicListings, setPublicListings] = useState([]) // 承認済み求人（求職者向け表示用・全画面共通で購読）
  const [selectedJob, setSelectedJob] = useState(null) // 求人詳細ページで表示中の求人
  ...
  // reCAPTCHA v3のスクリプトを読み込む（掲載申込フォームの不正送信対策）
  useEffect(() => {
  ...
  // Firebase Authのログイン状態を監視（ブラウザを閉じても再ログイン不要）
  useEffect(() => {
  ...
  // 承認済み求人を常時リアルタイム購読（求職者向けページ全体で使用）
  // ※ status絞り込み＋createdAt並び替えを同時にFirestoreへ投げると複合インデックスが必要になるため、
  //    絞り込みのみFirestoreに任せ、並び替えは受信後にブラウザ側で行う（インデックス設定不要にするため）
  useEffect(() => {
    const q = query(
      collection(db, 'listingRequests'),
      where('status', '==', 'approved')
    )
    // 表示優先度：トップ特別広告 > エリア有料枠 > 無料掲載。同じ優先度内では新しい順
    const PLAN_ORDER = { premium: 0, area: 1, free: 2 }
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        setPublicListings(jobs)
      },
      (err) => {
        console.error('公開求人一覧の取得に失敗しました', err)
      }
    )
    return () => unsubscribe()
  }, [])
  ...
  const handleAdminLogin = async (e) => {
    ...
    try {
      ...
      console.error('管理画面ログインに失敗しました', err)
      setAdminError('メールアドレスまたはパスワードが違います')
    } finally {
  ...
  // ナビゲーションリンク用：ホーム以外の画面にいる場合はホームへ切り替えてから対象セクションへスクロール
  ...
  // 求人詳細から戻る（一覧ページ経由なら一覧へ、トップの新着案件からなら一覧が無いのでホームへ）
  const handleBackFromJobDetail = () => {
    setSelectedJob(null)
    setView(listingSelection ? 'listing' : 'home')
  }
  ...
  const handlePostSubmit = async (e) => {
    ...
    setPostError('')
    try {
      // reCAPTCHA v3のトークンを取得（不正送信対策）。取得できない場合はサーバー側で弾かれる
      let recaptchaToken = ''
      try {
        ...
        recaptchaToken = await window.grecaptcha.execute(RECAPTCHA_SITE_KEY, { action: 'submit_listing' })
      } catch (recaptchaErr) {
        console.error('reCAPTCHAトークンの取得に失敗しました', recaptchaErr)
      }

      // サーバー側（api/create-listing.js）でreCAPTCHA検証後にFirestoreへ書き込む。
      // 通信不良等で無応答のまま保留し続けないよう、一定時間で強制的にタイムアウトさせる
      const writePromise = fetch('/api/create-listing', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          invoiceRequired: postForm.invoiceRequired,
        ...
        if (!r.ok || !data.listingId) {
          throw new Error(data.error || 'create_listing_failed')
        }
        return data
      })
      ...
      const { listingId } = await Promise.race([writePromise, timeoutPromise])

      // AI審査を実行する（失敗しても申込自体は成立させるため、エラーは握りつぶしてログのみ）
      // OK判定→自動承認、グレー/NG判定→LINE通知、という振り分けはscreen-listing.js側で行う
      try {
        ...
        await fetch('/api/screen-listing', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            listingId,
        ...
        console.error('AI審査の呼び出しでエラーが発生しました（申込自体は正常に完了しています）', screeningErr)
      }
        ...
        <>
          ...
          <div className="kk-admin-login-wrap">
            <form className="kk-admin-login-box" onSubmit={handleAdminLogin}>
              ...
              <button type="submit" className="kk-btn-post" disabled={adminLoggingIn}>
                {adminLoggingIn ? 'ログイン中…' : 'ログイン'}
              </button>
            </form>
          </div>
        </>
        ...
        <div className="kk-admin-wrap">
          ...
          {filteredListings.map(listing => (
            <div key={listing.id} className="kk-admin-card">
              ...
              {listing.aiScreening && listing.aiScreening !== 'ok' && (
                <div className={`kk-ai-screening-badge kk-ai-screening-${listing.aiScreening}`}>
                  {listing.aiScreening === 'ng' ? '🚫 AI審査：NG（自動却下）' : '⚠️ AI審査：グレー（要確認）'}
                  {listing.aiScreeningReason && ` — ${listing.aiScreeningReason}`}
                </div>
              )}
              ...
              <div className="kk-admin-card-row">
                ...
                　インボイス：{listing.invoiceRequired === 'required' ? '必要' : listing.invoiceRequired === 'not_required' ? '必要なし' : '未回答'}
              </div>
  ...
  return (
    <>
      ...
      <div className="kk-app">
        ...
        {/* ③ プレミアム広告（トップ特別広告プランの実掲載を表示。掲載が無い場合は非表示） */}
        {(() => {
          const premiumJob = publicListings.find(j => j.plan === 'premium')
          if (!premiumJob) return null
          return (
            <div className="kk-premium-ad" onClick={()=>openJobDetail(premiumJob)}>
              <div className="kk-premium-label">★ プレミアム広告（全国1社限定）</div>
              <div className="kk-premium-company">{premiumJob.companyName}</div>
              {premiumJob.jobTitle && <div className="kk-premium-jobtitle">{premiumJob.jobTitle}</div>}
              <div className="kk-premium-highlights">
                <span className="kk-premium-chip">📍 {getAreaLabel(premiumJob)}</span>
                {premiumJob.payRate && <span className="kk-premium-chip kk-premium-chip-pay">💰 {premiumJob.payRate}</span>}
              </div>
              <div className="kk-premium-desc">▶ 詳しく見る</div>
            </div>
          )
        })()}
        ...
        <nav className="kk-nav">
          <div className="kk-nav-links">
            <a href="#area" onClick={(e)=>{e.preventDefault(); scrollToSection('area')}}>エリアから探す</a>
            <a href="#jobs" onClick={(e)=>{e.preventDefault(); scrollToSection('jobs')}}>新着案件</a>
            <a href="#about" onClick={(e)=>{e.preventDefault(); scrollToSection('about')}}>掲載について</a>
            <a href="#price" onClick={(e)=>{e.preventDefault(); scrollToSection('price')}}>料金プラン</a>
          </div>
        ...
        <section id="area" className="kk-area-section">
          <div className="kk-section-title">
            エリアから探す
            <span className="kk-total-count">全国　求人案件 {publicListings.length.toLocaleString()}件</span>
          </div>
          ...
          <div className="kk-region-list">
            {regions.map(region => (
              <div key={region.id} className={`kk-region-item ${selectedRegion?.id===region.id?'active':''}`} onClick={()=>handleRegionClick(region)}>
                ...
                <span className="kk-region-count">{publicListings.filter(j=>j.regionId===region.id).length.toLocaleString()}件</span>
              </div>
            ))}
          </div>
              ...
              <div className="kk-pref-grid">
                {selectedRegion.prefectures.map(pref => (
                  <div key={pref.id} className={`kk-pref-btn ${selectedPref?.id===pref.id?'active':''}`} onClick={()=>handlePrefClick(pref, selectedRegion)}>
                    <div>{pref.name}</div>
                    <div className="kk-pref-count">{publicListings.filter(j=>j.prefId===pref.id).length}件</div>
                  </div>
                ))}
              </div>
                  ...
                  <div className="kk-area-grid">
                    {selectedPref.areas.map(area => (
                      <div key={area.id} className="kk-area-btn" onClick={()=>handleAreaClick(area, selectedRegion, selectedPref)}>
                        <div>{area.name}</div>
                        <div className="kk-area-count">{publicListings.filter(j=>j.areaId===area.id).length}件</div>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <section id="jobs" className="kk-jobs-section">
          <div className="kk-section-title">新着案件</div>
          {publicListings.length === 0 ? (
            <div className="kk-listing-empty">
              <p>現在、掲載中の求人はありません。近日公開予定です！</p>
            </div>
          ) : (
            publicListings.slice(0, 5).map((job) => (
              <div key={job.id} className="kk-job-card" onClick={()=>openJobDetail(job)}>
                <div className="kk-job-header">
                  <span className="kk-job-title">{job.jobTitle || job.companyName}</span>
                  <span className={`kk-tag ${job.plan!=='free'?'kk-tag-paid':'kk-tag-free'}`}>{job.plan!=='free'?'有料':'無料'}</span>
                </div>
                <div className="kk-job-meta">{job.companyAddress}　{job.payRate}</div>
              </div>
            ))
          )}
        </section>
        ...
        {/* ⑧ 掲載について */}
        <section id="about" className="kk-about-section">
          <div className="kk-section-title">掲載について</div>
          <div className="kk-about-box">
            <p className="kk-detail-text">
              軽貨物求人.comは、軽貨物ドライバーの求人を専門に扱う掲載サイトです。無料掲載プランをご用意しているため、まずは費用をかけずに求人を掲載いただけます。
            </p>
            <ul className="kk-about-list">
              <li>掲載申込みから公開まで、通常◯時間以内に運営側で内容を確認のうえ公開します</li>
              <li>会社名・所在地・連絡先・仕事内容・報酬条件など、求職者が判断に必要な情報を具体的にご記入ください</li>
              <li>同じ会社で複数のエリアに対応している場合は、エリアごとに掲載申込みが可能です</li>
              <li>掲載内容に虚偽や不適切な内容が含まれる場合、公開をお断り、または公開後に停止する場合があります</li>
            </ul>
            <button className="kk-btn-post" onClick={()=>openPostForm()}>案件を掲載する</button>
          </div>
        </section>
        ...
        {/* ⑨ 料金プラン */}
        <section id="price" className="kk-price-section">
          <div className="kk-section-title">料金プラン</div>
          <div className="kk-price-grid">
            <div className="kk-price-card">
              <div className="kk-price-name">無料掲載</div>
              <div className="kk-price-amount">¥0</div>
              <ul className="kk-price-features">
                <li>基本情報での掲載</li>
                <li>エリア一覧・新着案件に表示</li>
              </ul>
            </div>
            <div className="kk-price-card">
              <div className="kk-price-name">エリア有料枠</div>
              <div className="kk-price-amount">¥10,000<span className="kk-price-unit">/月・税抜</span></div>
              <ul className="kk-price-features">
                <li>エリアごとのお申込み</li>
                <li>無料掲載より上位に表示</li>
                <li>複数エリア対応の場合はエリアごとにお申込みいただけます</li>
              </ul>
            </div>
            <div className="kk-price-card kk-price-card-premium">
              <div className="kk-price-name">トップ特別広告</div>
              <div className="kk-price-amount">¥50,000<span className="kk-price-unit">/月・税抜</span></div>
              <ul className="kk-price-features">
                <li>全国1社限定</li>
                <li>トップページ最上部に掲載</li>
                <li>最も高い訴求効果</li>
          ...
          <p className="kk-detail-text">料金はすべて月額固定で、翌月1日開始・日割り計算はございません。お申込み・お支払いはオンライン決済（Stripe）にて完結します。</p>
        </section>
        </>
        )}
        ...
        {view === 'listing' && listingSelection && (
          <section id="listing" className="kk-listing-section">
            ...
            {(() => {
              const jobs = listingSelection.area
                ? publicListings.filter(j => j.areaId === listingSelection.area.id)
                : publicListings.filter(j => j.prefId === listingSelection.pref.id)
              if (jobs.length === 0) {
                return (
                  <div className="kk-listing-empty">
        ...
        {view === 'jobDetail' && selectedJob && (
          <section id="jobDetail" className="kk-jobdetail-section">
            <button className="kk-back-btn" onClick={handleBackFromJobDetail}>← 一覧に戻る</button>

            <div className="kk-jobdetail-header">
              <span className={`kk-tag ${selectedJob.plan!=='free'?'kk-tag-paid':'kk-tag-free'}`}>{selectedJob.plan!=='free'?'有料掲載':'無料掲載'}</span>
              <h1 className="kk-jobdetail-title">{selectedJob.jobTitle || selectedJob.companyName}</h1>
              <div className="kk-jobdetail-company">{selectedJob.companyName}</div>
            </div>

            <div className="kk-form-section-title">基本情報</div>
            <div className="kk-detail-row"><span className="kk-detail-label">勤務地</span><span>{selectedJob.workLocation || selectedJob.companyAddress || '未記載'}</span></div>
            <div className="kk-detail-row"><span className="kk-detail-label">契約形態</span><span>{(selectedJob.jobTypes||[]).map(id => JOB_TYPE_OPTIONS.find(o=>o.id===id)?.label).filter(Boolean).join('・') || selectedJob.jobTypeOther || '未記載'}</span></div>
            <div className="kk-detail-row"><span className="kk-detail-label">勤務日数</span><span>{(selectedJob.workDays||[]).map(id => WORK_DAYS_OPTIONS.find(o=>o.id===id)?.label).filter(Boolean).join('・') || '未記載'}</span></div>

            <div className="kk-form-section-title">報酬・契約条件</div>
            <div className="kk-detail-row"><span className="kk-detail-label">報酬</span><span>{selectedJob.payRate || '未記載'}</span></div>
            {selectedJob.monthlyIncomeExample && <div className="kk-detail-row"><span className="kk-detail-label">月収例</span><span>{selectedJob.monthlyIncomeExample}</span></div>}
            {selectedJob.dailyGuarantee && <div className="kk-detail-row"><span className="kk-detail-label">日当保証</span><span>{selectedJob.dailyGuarantee}</span></div>}
            <div className="kk-detail-row"><span className="kk-detail-label">支払条件</span><span>{(selectedJob.paymentTermsList||[]).map(id => PAYMENT_TERMS_OPTIONS.find(o=>o.id===id)?.label).filter(Boolean).join('・') || selectedJob.paymentTermsOther || '未記載'}</span></div>
            <div className="kk-detail-row"><span className="kk-detail-label">ロイヤリティ・管理費</span><span>{selectedJob.hasRoyalty === 'yes' ? <span className="kk-royalty-yes">あり</span> : selectedJob.hasRoyalty === 'no' ? 'なし' : '未回答'}</span></div>
            <div className="kk-detail-row"><span className="kk-detail-label">インボイス</span><span>{selectedJob.invoiceRequired === 'required' ? '必要' : selectedJob.invoiceRequired === 'not_required' ? '必要なし' : '未回答'}</span></div>
            {(selectedJob.vehicleCondition || selectedJob.vehicleConditionDetail) && (
              <div className="kk-detail-row"><span className="kk-detail-label">車両条件</span><span>{VEHICLE_CONDITION_LABELS[selectedJob.vehicleCondition] || selectedJob.vehicleCondition}{selectedJob.vehicleConditionDetail ? '　'+selectedJob.vehicleConditionDetail : ''}</span></div>
            )}

            {selectedJob.jobDescription && (<>
              <div className="kk-form-section-title">仕事内容</div>
              <p className="kk-detail-text">{selectedJob.jobDescription}</p>
            </>)}
            {selectedJob.benefits && (<>
              <div className="kk-form-section-title">待遇・福利厚生</div>
              <p className="kk-detail-text">{selectedJob.benefits}</p>
            </>)}
            {selectedJob.qualifications && (<>
              <div className="kk-form-section-title">資格・学歴</div>
              <p className="kk-detail-text">{selectedJob.qualifications}</p>
            </>)}
            {selectedJob.requirements && (<>
              <div className="kk-form-section-title">必須条件</div>
              <p className="kk-detail-text">{selectedJob.requirements}</p>
            </>)}
            {selectedJob.businessDescription && (<>
              <div className="kk-form-section-title">企業について</div>
              <p className="kk-detail-text">{selectedJob.businessDescription}</p>
            </>)}

            <div className="kk-form-section-title">応募について</div>
            <p className="kk-detail-text">この求人にご興味のある方は、以下より企業へ直接お問い合わせください。</p>
            <div className="kk-jobdetail-contact">
              {selectedJob.phone && <a className="kk-btn-post" href={`tel:${selectedJob.phone}`}>電話で問い合わせる（{selectedJob.phone}）</a>}
              {selectedJob.email && <a className="kk-btn-post kk-btn-post-outline" href={`mailto:${selectedJob.email}?subject=${encodeURIComponent('【軽貨物求人.com】'+(selectedJob.jobTitle||'求人')+'について')}`}>メールで問い合わせる</a>}
            </div>
          </section>
        )}
          ...
          <section id="postForm" className="kk-postform-section">
            ...
            {postStatus === 'success' ? (
              ...
              <form className="kk-postform" onSubmit={handlePostSubmit}>
                ...
                <div className="kk-form-group">
                  <label>インボイス<span className="kk-required">必須</span></label>
                  <select required value={postForm.invoiceRequired} onChange={e=>updatePostForm('invoiceRequired', e.target.value)}>
                    <option value="">選択してください</option>
                    <option value="required">必要</option>
                    <option value="not_required">必要なし</option>
                  </select>
                </div>
