...
### 追加
- `api/screen-listing.js` を再作成。GitHub上から実体が消失していたためLINE通知が届かなくなっていた（原因未特定）。HANDOVER.mdに記録されていた仕様（OK/グレー/NG判定、`ANTHROPIC_API_KEY`未設定時は安全側にグレー判定へフォールバック）に基づき復元

### 変更
- 管理画面の認証を暫定パスワード方式からFirebase Authentication（メール/パスワード）へ移行。`ADMIN_PASSWORD`定数を廃止（`DECISIONS.md` D-007）
- Firestoreセキュリティルールを全開放（`allow read, write: if true;`）から変更。`listingRequests`コレクションは新規作成のみ誰でも可、閲覧・更新・削除は認証済み管理者のみに制限（`DECISIONS.md` D-008）

### 修正
- ルート階層に残っていた重複ファイル`screen-listing.js`（`api/`配下ではない方）を削除

### 確認
- Firebase Authでの管理画面ログイン成功
- 掲載フォーム送信→AI審査（グレー判定）→LINE通知→承認ボタンでのステータス自動更新→管理画面反映、の一連の流れを再確認
